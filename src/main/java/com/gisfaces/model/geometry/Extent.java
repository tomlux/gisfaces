 ...
 * @see <a href="https://developers.arcgis.com/javascript/latest/api-reference/esri-geometry-Extent.html">ESRI Extent</a>
 */
public class Extent implements Geometry, Serializable
{
	...
	public static final Extent union(List<Point> points)
	{
		...
		if ((points != null) && (!points.isEmpty()))
		{
			...
			for (Point c : points)
			{
				if (c.getLatitude() < e.getYmin())	e.setYmin(c.getLatitude());
				if (c.getLatitude() > e.getYmax())	e.setYmax(c.getLatitude());
				if (c.getLongitude() < e.getXmin())	e.setXmin(c.getLongitude());
				if (c.getLongitude() > e.getXmax())	e.setXmax(c.getLongitude());
			}
		}
